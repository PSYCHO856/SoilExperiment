...
namespace LTGame
{
    ...
    public class LTEditorWindow : EditorWindow
    {
        ...
        private void OnGUI()
        {
            ...
            _toggleIndex = 0;
            //_HandleToggleUI("Excel配置自动生成_TS", _ExcelHandle);
            _HandleToggleUI("Excel配置自动生成_CS", _ExcelHandle);
        ...
        {
            LTEditorUtils.ShowLine("Config命名空间", ref LTEditorData.instance.excelNameSpace, "MainGame", false);
            LTEditorUtils.ShowLine("Excel保存路径", ref LTEditorData.instance.excelSavePath, "");
            LTEditorUtils.ShowLine("Config代码储存路径", ref LTEditorData.instance.configCodeSavePath, "/Scripts/GenConfig");
            LTEditorUtils.ShowLine("Config文件储存路径", ref LTEditorData.instance.configDataSavePath, "/data/config");
