...
namespace SuperScrollView
{
    ...
    public class LoopListView2 : MonoBehaviour, IBeginDragHandler, IEndDragHandler, IDragHandler
    {
        ...
        */
        ...
        public void ClearAllRecycledItem()
        {
            mCurReadyMaxItemIndex = 0;
            mCurReadyMinItemIndex = 0;
            mNeedCheckNextMaxItem = false;
        ...
        void AdjustPanelPos()
        {
            ...
            if (mArrangeType == ListItemArrangeType.TopToBottom)
            {
                if (contentSize <= viewPortSize)
                {
                    ...
                    mItemList[0].CachedRectTransform.anchoredPosition3D = new UnityEngine.Vector3(mItemList[0].StartPosOffset,0,0);
                    UpdateAllShownItemsPos();
                    return;
                }
            ...
            else if (mArrangeType == ListItemArrangeType.LeftToRight)
            {
                if (contentSize <= viewPortSize)
                {
                    ...
                    pos.x = 0;
                    mContainerTrans.anchoredPosition3D = pos;
                    mItemList[0].CachedRectTransform.anchoredPosition3D = new UnityEngine.Vector3(0, mItemList[0].StartPosOffset, 0);
                    UpdateAllShownItemsPos();
                    return;
                }
                LoopListViewItem2 tViewItem0 = mItemList[0];
        ...
        bool UpdateForVertList(float distanceForRecycle0,float distanceForRecycle1,float distanceForNew0, float distanceForNew1)
        {
            ...
            if (mArrangeType == ListItemArrangeType.TopToBottom)
            {
                ...
                if (itemListCount == 0)
                {
                    ...
                    mItemList.Add(newItem);
                    newItem.CachedRectTransform.anchoredPosition3D = new UnityEngine.Vector3(newItem.StartPosOffset, pos, 0);
                    UpdateContentSize();
                    return true;
                }
                LoopListViewItem2 tViewItem0 = mItemList[0];
                ...
                if (mViewPortRectLocalCorners[0].y - downPos1.y < distanceForNew1)
                {
                    ...
                    if (nIndex <= mCurReadyMaxItemIndex || mNeedCheckNextMaxItem)
                    {
                        ...
                        else
                        {
                            ...
                            newItem.CachedRectTransform.anchoredPosition3D = new UnityEngine.Vector3(newItem.StartPosOffset, y, 0);
                            UpdateContentSize();
                            CheckIfNeedUpdataItemPos();
        ...
        bool UpdateForHorizontalList(float distanceForRecycle0, float distanceForRecycle1, float distanceForNew0, float distanceForNew1)
        {
            ...
            if (mArrangeType == ListItemArrangeType.LeftToRight)
            {
                ...
                if (mItemList.Count == 0)
                {
                    ...
                    mItemList.Add(newItem);
                    newItem.CachedRectTransform.anchoredPosition3D = new UnityEngine.Vector3(pos, newItem.StartPosOffset, 0);
                    UpdateContentSize();
                    return true;
                }
                LoopListViewItem2 tViewItem0 = mItemList[0];
                ...
                if (rightPos1.x - mViewPortRectLocalCorners[2].x < distanceForNew1)
                {
                    ...
                    if (nIndex <= mCurReadyMaxItemIndex || mNeedCheckNextMaxItem)
                    {
                        ...
                        else
                        {
                            ...
                            newItem.CachedRectTransform.anchoredPosition3D = new UnityEngine.Vector3(x, newItem.StartPosOffset, 0);
                            UpdateContentSize();
                            CheckIfNeedUpdataItemPos();
        ...
        void UpdateAllShownItemsPos()
        {
            ...
            if (mArrangeType == ListItemArrangeType.TopToBottom)
            {
                ...
                for (int i = 0; i < count; ++i)
                {
                    ...
                    item.CachedRectTransform.anchoredPosition3D = new UnityEngine.Vector3(item.StartPosOffset, curY, 0);
            ...
            else if (mArrangeType == ListItemArrangeType.LeftToRight)
            {
                ...
                {
                    LoopListViewItem2 item = mItemList[i];
                    item.CachedRectTransform.anchoredPosition3D = new UnityEngine.Vector3(curX, item.StartPosOffset, 0);
