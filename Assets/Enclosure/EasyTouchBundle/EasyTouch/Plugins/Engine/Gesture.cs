...
public class Gesture : BaseFinger,ICloneable{
	...
	public Vector3 GetTouchToWorldPoint(float z){
		...
		return Camera.main.ScreenToWorldPoint( new UnityEngine.Vector3(position.x, position.y, z));	

	}
	
	public Vector3 GetTouchToWorldPoint( Vector3 position3D){

		return Camera.main.ScreenToWorldPoint( new UnityEngine.Vector3(position.x, position.y, Camera.main.transform.InverseTransformPoint(position3D).z));	
	}
