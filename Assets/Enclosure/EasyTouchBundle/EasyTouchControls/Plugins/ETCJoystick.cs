...
public class ETCJoystick : ETCBase,IPointerEnterHandler,IDragHandler, IBeginDragHandler, IPointerDownHandler, IPointerUpHandler {
	...
	public override void Start(){
		...
		if (enableCamera && cameraMode == CameraMode.SmoothFollow){
			if (cameraTransform && cameraLookAt){
                cameraTransform.position = cameraLookAt.TransformPoint( new UnityEngine.Vector3(0, followHeight, -followDistance));
	...
	#region UI Callback
	public bool isDefultPosition = true;
	public void OnPointerEnter(PointerEventData eventData){

        if (joystickType == JoystickType.Dynamic && !isDynamicActif && _activated && pointId == -1 && !isDefultPosition)
        {
	...
	private void DoTurnAndMove(){
		...
		if (axisX.directTransform != null){
            ...
            axisX.directTransform.rotation = Quaternion.Euler(new UnityEngine.Vector3(0,  angle + tmAdditionnalRotation, 0));
