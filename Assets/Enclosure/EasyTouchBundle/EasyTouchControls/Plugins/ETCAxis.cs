...
public class ETCAxis {
	...
	public void DoGravity(){
		...
		if (directCharacterController != null && gravity!=0){
			...
			if (!isJump){
                Vector3 move = new UnityEngine.Vector3(0,-gravity, 0);
				directCharacterController.Move( move * Time.deltaTime);
			}
			else{
				currentGravity -= gravity*Time.deltaTime;
                Vector3 move = new UnityEngine.Vector3(0, currentGravity, 0);
				directCharacterController.Move( move * Time.deltaTime);
			}
	...
	private void DoAutoStabilisation(){
		...
		if (angle > startAngle - autoStabThreshold || angle < startAngle + autoStabThreshold){
			...
			switch(axisInfluenced){
			case AxisInfluenced.X:	
				stabAngle = new UnityEngine.Vector3(axis, _directTransform.localRotation.eulerAngles.y, _directTransform.localRotation.eulerAngles.z);
				break;
			case AxisInfluenced.Y:	
				stabAngle = new UnityEngine.Vector3(_directTransform.localRotation.eulerAngles.x, axis, _directTransform.localRotation.eulerAngles.z);
				break;
			case AxisInfluenced.Z:	
				stabAngle = new UnityEngine.Vector3(_directTransform.localRotation.eulerAngles.x, _directTransform.localRotation.eulerAngles.y, axis);
				break;
			}
