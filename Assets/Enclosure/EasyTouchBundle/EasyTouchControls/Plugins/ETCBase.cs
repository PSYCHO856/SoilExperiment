...
public abstract class ETCBase : MonoBehaviour {
	...
	public Vector3 followOffset = new UnityEngine.Vector3(0,6,-6);
	...
	protected void CameraSmoothFollow(){
		...
		newPos = new UnityEngine.Vector3(newPos.x , currentHeight, newPos.z);

		if (enableWallDetection){
			RaycastHit wallHit;

			if (Physics.Linecast( new UnityEngine.Vector3(cameraLookAt.position.x, cameraLookAt.position.y+1f, cameraLookAt.position.z), newPos, out wallHit)){
				newPos= new UnityEngine.Vector3( wallHit.point.x, currentHeight, wallHit.point.z);
			}
		}
