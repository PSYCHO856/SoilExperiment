...
/// 工具类
/// </summary>
public class UtilityTool : APP.Core.MonoSingleton<UtilityTool>
{
     
    public static void GetPos(RectTransform target, RectTransform source){
        RectTransformUtility.CalculateRelativeRectTransformBounds(target,source);
    }
    ...
    public IEnumerator LoadAsync(string name, Action<UnityEngine.Object> Callback)
    {
    ...
    {
        var dir = new UnityEngine.Vector3(pos.x, transform.position.y, pos.z) - transform.position;
    ...
    public static Vector3 GetPosV3(string pos) {
        string[] poss = pos.Split(',');
        return new UnityEngine.Vector3(Convert.ToSingle(poss[0]), Convert.ToSingle(poss[1]), Convert.ToSingle(poss[2]));
    }
    ...
    public static void BeHitEffect(List<Material> mats,List<Shader> orignalsShaders,int frameCount = 3)
    {
        ...
        // }
        //  UtilityTool.Instance.WaitForFrame(frameCount, () =>
        // {
    ...
    /// <returns></returns>
    public static Color GetQuality(string quality)
    {
        switch (quality)
        {
            case "0":
            ...
            case "4":
                return Color.yellow;
            case "5":
                return new Color(1,140/255,0);
        }

        return Color.white;
    }

    public void Begin(float coolTime, Action voidFunc)
    {
        StartCoroutine(Polling(coolTime, voidFunc));
    }
    ...
    private IEnumerator Polling(float delay, Action voidFunc)
    {
        while (delay > 0)
        {
            //但是需要每帧减少剩余时间，因为下面是yield return null，每帧执行一次，所以每帧减去这个Time.deltaTime才是比较严格的倒计时，不会跟现实时间对不上
            delay -= Time.deltaTime;
            //时间到则执行完成的回调
            if (delay < 0 && voidFunc != null)
            {
                voidFunc?.Invoke();
                yield break;
            }
    ...
    /// <param name="interval"></param>
    /// <param name="voidFunc"></param>
    public void BeginCount(float coolTim, Action<float> voidFunce,float interval=1,bool isReverse=false)
    {
        StartCoroutine(Pollings(coolTim,voidFunce,interval,isReverse));
    }
    public void StopCount()
    {
        StopCoroutine("Pollings");
    }
    private IEnumerator Pollings(float delay,Action<float> voidFunc,float interval,bool isReverse)
    {
        while (true)
        {
            //但是需要每帧减少剩余时间，因为下面是yield return null，每帧执行一次，所以每帧减去这个Time.deltaTime才是比较严格的倒计时，不会跟现实时间对不上
